/* ===================================================
   NexGen Dubai — blog.js
   Shared script: blog.html + blog-canada.html
=================================================== */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ════════════════════════════════
     BLOG GRID — search + filter
  ════════════════════════════════ */
  var cards = document.querySelectorAll('.bg-card');
  if (cards.length) {

    var filterBtns = document.querySelectorAll('.bg-filter-btn');
    var searchInput = document.getElementById('bg-search');
    var emptyMsg = document.getElementById('bg-empty');
    ...
    function filterCards() {
      ...
      cards.forEach(function(card){
        ...
        var text = (card.querySelector('.bg-card-title') ? card.querySelector('.bg-card-title').textContent : '')
                 + ' ' + (card.querySelector('.bg-card-excerpt') ? card.querySelector('.bg-card-excerpt').textContent : '');
        var matchCat = activeFilter === 'all' || cat === activeFilter;
        var matchSearch = !q || text.toLowerCase().includes(q);
    ...
    cards.forEach(function(card, i){
      card.style.opacity='0'; card.style.transform='translateY(32px)';
      var obs = new IntersectionObserver(function(entries){
        entries.forEach(function(e){
          if(e.isIntersecting){
            setTimeout(function(){ card.style.transition='opacity .65s ease,transform .65s ease,box-shadow .4s,border-color .4s'; card.style.opacity='1'; card.style.transform=''; },i*90);
            obs.unobserve(card);
          }
        });
      },{threshold:.1});
  ...
  /* Reading progress glow on hero title */
  var heroTitle = document.querySelector('.bp-hero-title');
  if(heroTitle) {
    heroTitle.style.opacity='0'; heroTitle.style.transform='translateY(24px)';
    setTimeout(function(){ heroTitle.style.transition='opacity .9s ease,transform .9s ease'; heroTitle.style.opacity='1'; heroTitle.style.transform=''; },200);
  }
