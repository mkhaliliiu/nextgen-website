...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var formWrap = document.getElementById('cr-form-wrap');
  var successWrap = document.getElementById('cr-success');
  var form = document.getElementById('cr-form');

  function getPanel(n)    { return document.getElementById('cr-step-'+n); }
  function getStepItem(n) { return document.getElementById('cr-si-'+n); }
  ...
  function updateUI(step) {
    ...
    document.querySelectorAll('.cr-step-line').forEach(function(l,idx){
      l.style.background = idx<step-1 ? 'rgba(255,255,255,.55)' : 'rgba(255,255,255,.2)';
    });
  }

  function showStep(step) {
    for (var i=1;i<=TOTAL;i++) { var p=getPanel(i); if(p) p.classList.remove('active'); }
    var t=getPanel(step);
    if(t) {
      t.classList.add('active');
      if(window.innerWidth<900) {
        var card=document.querySelector('.cr-form-card');
        if(card) setTimeout(function(){ card.scrollIntoView({behavior:'smooth',block:'start'}); },80);
      }
    }
    updateUI(step); current=step;
  }
  ...
  function validate(step) {
    ...
    panel.querySelectorAll('[required]').forEach(function(f){
      var err=f.closest('.cr-field') ? f.closest('.cr-field').querySelector('.cr-error-msg') : null;
      f.classList.remove('error'); if(err) err.classList.remove('show');
      if(!f.value||f.value.trim()==='') { f.classList.add('error'); if(err) err.classList.add('show'); valid=false; }
    });
    /* Require at least one interest on step 2 */
    if(step===2) {
      var any=panel.querySelector('.cr-interest-opt.checked');
      if(!any) {
        var wrap=panel.querySelector('.cr-interest-grid');
        if(wrap) { wrap.style.outline='2px solid #ef4444'; wrap.style.borderRadius='10px'; setTimeout(function(){ wrap.style.outline=''; },2000); }
        valid=false;
      }
    }
    return valid;
  ...
  document.querySelectorAll('.cr-btn-next').forEach(function(btn){
    btn.addEventListener('click', function(){ if(!validate(current)) return; if(current<TOTAL) showStep(current+1); });
  });
  document.querySelectorAll('.cr-btn-prev').forEach(function(btn){
    btn.addEventListener('click', function(){ if(current>1) showStep(current-1); });
  });

  /* Availability radio */
  document.querySelectorAll('.cr-avail-opt').forEach(function(opt){
    opt.addEventListener('click', function(e){
      e.preventDefault();
      document.querySelectorAll('.cr-avail-opt').forEach(function(o){ o.classList.remove('selected'); });
  ...
  /* Interest checkboxes */
  document.querySelectorAll('.cr-interest-opt').forEach(function(opt){
    opt.addEventListener('click', function(e){
      e.preventDefault();
      var cb=opt.querySelector('input[type="checkbox"]');
      var now=!opt.classList.contains('checked');
      opt.classList.toggle('checked',now);
      if(cb) cb.checked=now;
    });
  });

  /* Radio groups */
  document.querySelectorAll('.cr-radio-opt').forEach(function(opt){
    opt.addEventListener('click', function(e){
      e.preventDefault();
      var r=opt.querySelector('input[type="radio"]'); if(r) r.checked=true;
      var grp=opt.closest('.cr-radio-group');
      if(grp) grp.querySelectorAll('.cr-radio-opt').forEach(function(o){ o.classList.remove('selected'); });
      opt.classList.add('selected');
    });
  });

  /* Clear errors */
  document.querySelectorAll('.cr-input,.cr-select,.cr-textarea').forEach(function(f){
    f.addEventListener('input',  function(){ f.classList.remove('error'); });
    f.addEventListener('change', function(){ f.classList.remove('error'); });
  });

  /* Char counters */
  [['cr-about','cr-about-count',600],['cr-why','cr-why-count',500]].forEach(function(trio){
    var ta=document.getElementById(trio[0]), cnt=document.getElementById(trio[1]);
    if(ta&&cnt) ta.addEventListener('input', function(){ cnt.textContent=ta.value.length+'/'+trio[2]; });
  });
  ...
  if(form) {
    form.addEventListener('submit', function(e){
      ...
      var btn=form.querySelector('.cr-btn-submit');
      if(btn){ btn.disabled=true; btn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Sending...'; }
      setTimeout(function(){
        if(formWrap) formWrap.style.display='none';
        if(successWrap){
          successWrap.style.display='block';
          var icon=successWrap.querySelector('.cr-success-icon');
