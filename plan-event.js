...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var successWrap = document.getElementById('pe-success');
  var form        = document.getElementById('pe-form');

  function getPanel(n)    { return document.getElementById('pe-step-'+n); }
  function getStepItem(n) { return document.getElementById('pe-si-'+n); }
  ...
  function updateStepUI(step) {
    ...
    document.querySelectorAll('.pe-step-line').forEach(function(line,idx){
  ...
  function showStep(step) {
    ...
    if (target) {
      ...
      if (window.innerWidth < 900) {
        var card = document.querySelector('.pe-form-card');
  ...
  function validateStep(step) {
    ...
    panel.querySelectorAll('[required]').forEach(function(field){
      var errMsg = field.closest('.pe-field') ? field.closest('.pe-field').querySelector('.pe-error-msg') : null;
    ...
    /* validate budget selection on step 2 */
    if (step === 2) {
      var budgetSelected = panel.querySelector('.pe-budget-opt.selected');
      if (!budgetSelected) {
        var budgetWrap = panel.querySelector('.pe-budget-grid');
        if (budgetWrap) {
  ...
  /* Next / Prev */
  document.querySelectorAll('.pe-btn-next').forEach(function(btn){
    btn.addEventListener('click', function(){ if(!validateStep(currentStep)) return; if(currentStep<TOTAL_STEPS) showStep(currentStep+1); });
  });
  document.querySelectorAll('.pe-btn-prev').forEach(function(btn){
    btn.addEventListener('click', function(){ if(currentStep>1) showStep(currentStep-1); });
  });

  /* Budget cards */
  document.querySelectorAll('.pe-budget-opt').forEach(function(opt){
    opt.addEventListener('click', function(){
      document.querySelectorAll('.pe-budget-opt').forEach(function(o){ o.classList.remove('selected'); });
  ...
  /* Service checkboxes */
  document.querySelectorAll('.pe-check-opt').forEach(function(opt){
    opt.addEventListener('click', function(){
  ...
  /* Radio groups (contact method + schedule call) */
  document.querySelectorAll('.pe-radio-opt').forEach(function(opt){
    opt.addEventListener('click', function(){
      var radio = opt.querySelector('input[type="radio"]');
      if (radio) radio.checked = true;
      var group = opt.closest('.pe-radio-group');
      if (group) group.querySelectorAll('.pe-radio-opt').forEach(function(o){ o.classList.remove('selected'); });
      opt.classList.add('selected');
    });
  });

  /* Clear errors on input */
  document.querySelectorAll('.pe-input,.pe-select,.pe-textarea').forEach(function(f){
    f.addEventListener('input',  function(){ f.classList.remove('error'); });
    f.addEventListener('change', function(){ f.classList.remove('error'); });
  });

  /* Live char counter for vision textarea */
  var visionTA  = document.getElementById('pe-vision');
  var visionCnt = document.getElementById('pe-vision-count');
  if (visionTA && visionCnt) {
    visionTA.addEventListener('input', function(){ visionCnt.textContent = visionTA.value.length+'/700'; });
  }
  ...
  if (form) {
    form.addEventListener('submit', function(e){
      ...
      var btn = form.querySelector('.pe-btn-submit');
      if (btn) { btn.disabled=true; btn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Sending...'; }
      setTimeout(function(){
        if (formWrap) formWrap.style.display='none';
        if (successWrap) {
          successWrap.style.display='block';
          var icon = successWrap.querySelector('.pe-success-icon');
