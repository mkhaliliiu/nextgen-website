...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* Mobile nav */
  var ham = document.getElementById('hamburger'), closeBtn = document.getElementById('mobile-nav-close');
  ...
  /* Hero card stagger */
  document.querySelectorAll('.mp-hcard').forEach(function (c, i) {
    c.style.opacity = '0'; c.style.transform = 'translateX(36px)';
    setTimeout(function () { c.style.transition = 'opacity .6s ease,transform .6s ease,border-color .35s,box-shadow .35s'; c.style.opacity = '1'; c.style.transform = ''; }, 550 + i * 130);
  });

  /* Service card stagger on scroll */
  var scGrid = document.querySelector('.mp-services-grid');
  if (scGrid) {
    var scCards = scGrid.querySelectorAll('.mp-scard');
    scCards.forEach(function (c) { c.style.opacity = '0'; c.style.transform = 'translateY(30px)'; });
    var scObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) {
          scCards.forEach(function (c, i) {
            setTimeout(function () { c.style.transition = 'opacity .65s ease,transform .65s ease,box-shadow .4s,border-color .4s'; c.style.opacity = '1'; c.style.transform = ''; }, i * 90);
          });
          scObs.unobserve(e.target);
        }
      });
    }, { threshold: 0.06 });
    scObs.observe(scGrid);
  }

  /* Platform icon bounce */
  document.querySelectorAll('.mp-platform-card,.mp-news-card').forEach(function (card) {
    card.addEventListener('mouseenter', function () {
      var icon = card.querySelector('.mp-platform-icon,.mp-news-icon i');
      if (icon) { icon.style.transform = 'scale(1.3) rotate(-8deg)'; icon.style.transition = 'transform .3s ease'; }
    });
  ...
  /* Collab stat counters (already handled by cObs above) */
